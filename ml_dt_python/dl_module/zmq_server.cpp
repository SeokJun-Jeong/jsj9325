...
{
    context_t context(1);
    socklen_t responder(context, socket_type::rep);
    const char *ipc_file_path="/tmp/zmq_server";
    if (unlink(ipc_file_path) == -1 && errno != ENOENT)
    {
        cerr << "Error removing existing IPC file : " << strerror(errno) << endl;
        return 1;
    }
    responder.bind("ipc://" + string(ipc_file_path));
    cout << "서버가 IPC소켓에 바인딩 되었습니다" << ipc_file_path << endl;
    while (true)
    {
        message_t request;
        ...
        if (result && result.value() > 0)
        {
            ...
            cout << "받은 메세지: " << received_msg << endl;
            string reply_msg = "메세지 수신됨" + received_msg; //echo메세지
