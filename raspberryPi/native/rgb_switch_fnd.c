...
// #define DATA_PIN   0  // 데이터 핀 (74HC595 DS)
// #define CLOCK_PIN  1  // 클록 핀 (74HC595 SHCP)
// #define LATCH_PIN  2  // 래치 핀 (74HC595 STCP)
// #define SWITCH_PIN 3  // 스위치 입력 핀
...
//         if (digitalRead(SWITCH_PIN) == LOW) {  // 스위치가 눌렸을 때
//             delay(50);  // 디바운스 처리
//             unsigned long pressTime = millis();

//             while (digitalRead(SWITCH_PIN) == LOW);  // 스위치가 눌려있는 시간 측정
//             pressTime = millis() - pressTime;

//             if (pressTime < 1000) value += 1;
//             else if (pressTime < 3000) value += 5;
//             else value += 10;

//             if (value > 99) value = 0;

//             uint16_t displayData = (fnd_data[value / 10] << 8) | fnd_data[value % 10];
...
// 3-> B 색 변화
// 스위치를 오래 누르면(0 ~ 1 초)-> (0~4095) 높은 값으로 변화 할 수 있게 만들기
//차이점 : rgb를 굳이 main에 넣지 않고 interrupt한거에 추가
...
#define DATA_PIN   0  // 데이터 핀 (74HC595 DS)
#define CLOCK_PIN  6  // 클록 핀 (74HC595 SHCP)
#define LATCH_PIN  5  // 래치 핀 (74HC595 STCP)
